...
#SBATCH --job-name=NegPerturbMUUWS
#SBATCH --mail-type=END
#SBATCH --mail-user=
#SBATCH --mem=3G
#SBATCH --cpus-per-task=2
#SBATCH --array=1-101
#SBATCH --output=outputs/UWS_Neg_Wgain_MU_A%A_%a.out
#SBATCH --error=outputs/UWS_Neg_Wgain_MU_A%A_%a.err

#Load Matlab 2017a module
ml MATLAB

matlab -nojvm -nodisplay<<-EOF
%struc = load("NotPassedBetasMUuws.mat");
...
gen=load('AAL/MU_carfentanil_hc204_kantonen.csv');
gen=gen(1:90);
...
for REP=1:20
    ...
    %%%
    mkdir(sprintf('Results/UWS_Neg_Wgain_MU/%02d',REP))
    save(sprintf('Results/UWS_Neg_Wgain_MU/%02d/job_%03d.mat',REP,s),'FCDfitt','GBCfitt','FCfitt','FC_simul','mFR','FC_emp');
